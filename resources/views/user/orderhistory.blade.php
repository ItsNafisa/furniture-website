...
<div class="container">
	...
	<div class="collapse navbar-collapse" id="navbarsFurni">
		<ul class="custom-navbar-nav navbar-nav ms-auto mb-2 mb-md-0">
			...
			@auth
			<li class="active"><a class="nav-link" href="{{url('order-history')}}">Order History</a></li>
			@endauth
		...
		<div class="untree_co-section before-footer-section">
          ...
          @if($orders->count() > 0)        
                    <table class="table">
                      ...
                      <tbody>
						<!-- start foreach -->
                        <tr class="cartsection">
                        <td class="product-name">
							<?php  $i++;?>
                            <h2 class="h5 text-black">{{$i}}</h2>
                          </td>
                          <td class="product-name">
                            <h2 class="h5 text-black">{{$order->total_price}}</h2>
                          </td>
						  <td class="product-name">
                            <h2 class="h5 text-black">{{$order->order_status=='0'?'Pending':'Completed'}}</h2>
                          </td>
					<td><a style="background-color:blue;padding:15px;color:white;text-decoration:none;border-radius:20px;cursor:pointer;" href="{{url('/view-order',$order->id)}}">View</a></td>     
                        </tr>
                      </tbody>
                 ...
                 @else
				<h3 class="text-center" style="font-weight:bold;"> No Order Yet</h3>
				 @endif
                  </div>
              </div>
